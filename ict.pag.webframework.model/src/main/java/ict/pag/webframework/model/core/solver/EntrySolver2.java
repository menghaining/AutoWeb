...
public class EntrySolver2 {
	private ClassHierarchy cha;
	private HashMap<String, HashSet<Element>> class2xmlEle;
	private Set<String> allUnreachableEntryPoints;
	...
	public EntrySolver2(Set<String> allUnreachableEntryPoints, ClassHierarchy cha0,
			HashMap<String, HashSet<Element>> class2xmlEle) {
		this.cha = cha0;
		this.class2xmlEle = class2xmlEle;
		this.allUnreachableEntryPoints = allUnreachableEntryPoints;
	 ...
	 */
	public void calculateEntry(HashSet<String> unreachableCalls) {
		...
		while (it.hasNext()) {
			...
			for (IMethod m : cn.getDeclaredMethods()) {
				...
				if (!name.equals("<init>") && !name.equals("<cinit>")) {
					if (allUnreachableEntryPoints.contains(m.getSignature())) {
						isAllCalled = false;
					} else {

...
public class ConfigUtil {
	...
	public static boolean enableSccs = false;

	// level 0 means all marks are equal;
	// level 1 means inheritance mark is the next priority
	public static int marksLevel = 0;
	...
	public HashSet<String> allApplications = new HashSet<>();
...
//	private ClassHierarchy cha_appWithLibs;
	...
	public void loadConfiguration(CLIOption cliOptions) {
		...
		outputFile = cliOptions.getOutputFile();
	}

	public void setApplicationClasses(HashSet<String> alls) {
		allApplications.addAll(alls);
	}
	...
	public static boolean isApplicationClass(String name) {
		...
		if (ConfigUtil.g().allApplications.contains(name))
			return true;
		else
			return false;
...
//			/** hello-web */
		...
		case "spring-petclinic":
			if (name.startsWith("org.springframework.samples."))
				return true;
			break;
		case "spring-mvc-showcase":
			if (name.startsWith("org.springframework.samples.mvc."))
				return true;
			break;
		}
		return false;
	...
	public static boolean isFrameworkMarks(String name) {
		...
		if (!ConfigUtil.isApplicationClass(name))
			return true;
		else
			return false;
...
////			return true;
////		if (name.startsWith("javax.beans."))
////			return true;
//
...
//		return true;
	...
	public static boolean notConcernedXML(String type) {
		if (type != null) {

...
public class OptimizeAnswer {
	...
	public static HashSet<EntryMark> resolveEntryAnswer(HashSet<EntryMark> entryMarkSet) {
		...
		// 1.merge 2.remove ignores
		boolean changed = mergeAndRemoveIgnores(entryMarkSet, ignore_class, ignore_mtd);
		while (changed)
			changed = mergeAndRemoveIgnores(entryMarkSet, ignore_class, ignore_mtd);
	...
	private static boolean mergeAndRemoveIgnores(HashSet<EntryMark> entryMarkSet, HashSet<String> ignore_class,
			HashSet<String> ignore_mtd) {
		...
		while (it0.hasNext()) {
			...
			while (it.hasNext()) {
				...
				boolean c1 = mark.mergeMarks(curr, ignore_class, ignore_mtd);
				if (c1)
					changed = true;
			}
			entryMarkSet_merged.add(mark);

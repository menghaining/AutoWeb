...
public class Callsite2CallSeqMapTool {
	...
	private HashMap<String, ArrayList<String>> callsite2itsActualCalls = new HashMap<>();
		...
		for (ArrayList<String> sequence : Allsequences) {
			for (int i = 0; i < sequence.size();) {
				...
				if (line.startsWith("[callsite]")) {
					...
					callsite2itsActualCalls.put(callStmt, actualCalls);
...
//							System.out.println();
	...
	public HashMap<FieldReference, String> getFieldRef2target_app_same() {
		return fieldRef2target_app_same;
	}
...
//			tmp.addAll(others2);
	...
	public HashMap<String, ArrayList<String>> getCallsite2itsActualCalls() {
		return callsite2itsActualCalls;
	}

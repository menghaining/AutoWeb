 ...
 */
public final class MethodEditorForBean {
  ...
  private final ShrikeBTMethod methodInfo;
   ...
   */
  public MethodEditorForBean(ShrikeBTMethod info) {
    if (info == null) {
    ...
    try {
		...
		System.err.println("construct methodeditor failure");
		e.printStackTrace();
	}
...
//    }
    ...
    if (methodInfo != null) {
      ...
      methodInfo.setTempIInstructions(instructions);
      try {
		methodInfo.setHandlers(handlers);
	} catch (InvalidClassFileException e1) {
		// TODO Auto-generated catch block
		e1.printStackTrace();
	}
      try {
		methodInfo.setpcMap(instructionsToBytecodes);
	} catch (InvalidClassFileException e) {
		// TODO Auto-generated catch block
		e.printStackTrace();
	}
  ...
  public static class Visitor extends IInstruction.Visitor {
    ...
    private MethodEditorForBean editor;

    /** Set the current editor and instruction index for this visitor. */
    public final void setIndex(MethodEditorForBean e, int i) {
      index = i;
      editor = e;
    }

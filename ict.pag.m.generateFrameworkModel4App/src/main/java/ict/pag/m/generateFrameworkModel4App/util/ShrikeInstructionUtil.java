 ...
 *
 * <p>
 * In the documentation below, we refer to a 'Java class name'. These are
 * formatted according to the rules for Class.forName() and Class.getName(). A
 * Java class name must use '$' to separate inner class names from their
 * containing class. There is no way to for Shrike to disambiguate 'A.B'
 * otherwise.
 */
public final class ShrikeInstructionUtil {
	private ShrikeInstructionUtil() {
	 ...
	 * parameters for method signature "type". Any "this" parameter is not included.
	 *
	...
	/**
	 * Convert a fully-qualified Java type name (either primitive or class name, '.'
	 * separated) into an internal JVM type name (one letter for primitive and '/'
	 * separated, starting with 'L' and ending with ';' for class name).
	 */
	...
	/**
	 * Convert a JVM type name (either for a primitive or a class name) into a Java
	 * type name.
	 */
	...
	public static final HashMap<String, String> typeAliases;
	...
	public static String makeType(IClass c) {
		if (c == null) {
			throw new IllegalArgumentException("c is null");
		}
	...
	/**
	 * Compute the number of parameters given by method signature "type". Any "this"
	 ...
	 *
	 * @param thisClassType null if the method is static, otherwise the type of
	 *                      "this"
	 ...
	 * "getParamsTypes" except null array entries are inserted to account for unused
	 * local variables because of 2-word parameter values.
	 *
	 * @throws IllegalArgumentException if type == null
	 */
	...
	/**
	 * Compute the promoted type that the JVM uses to manipulate values of type "t"
	 * on its working stack.
	 *
	 * @throws IllegalArgumentException if t is null
	 */
	...
	public static String computeSignature(IMethod m) throws IllegalArgumentException {
		StringBuilder buf = new StringBuilder();
		buf.append('(');
		for(int i=1;i<m.getNumberOfParameters();i++) {
			buf.append(makeType(m.getParameterType(i).getName().toString()));
		}
		buf.append(')');
		buf.append(makeType(m.getReturnType().getName().toString()));
		return buf.toString();
	}

	/**
	 * Make an Instruction which loads the value of a field, given its name and Java
	 * Class. The field type is obtained using reflection.
	 *
	 * @throws IllegalArgumentException if c is null
	 */
	public static GetInstruction makeGet(Class<?> c, String name) {
		...
		try {
			...
			return GetInstruction.make(makeType(f.getType()), makeType(c), name,
					(f.getModifiers() & Constants.ACC_STATIC) != 0);
	 ...
	 * @throws IllegalArgumentException if c is null
	 */
	public static PutInstruction makePutNoStatic(IClass c, IField name) {
		if (c == null) {
			throw new IllegalArgumentException("c is null");
		}
		try {
			return PutInstruction.make(makeType(name.getFieldTypeReference().getName().toString()), makeType(c),
					name.getName().toString(), false);
		} catch (SecurityException e) {
			throw new IllegalArgumentException(e.getMessage());
		}
	}

	public static PutInstruction makePutNoStatic(IClass c, String fieldTypename, String fieldName) {
		if (c == null) {
			throw new IllegalArgumentException("c is null");
		}
		try {
			return PutInstruction.make(makeType(fieldTypename), makeType(c), fieldName, false);
		} catch (SecurityException e) {
			throw new IllegalArgumentException(e.getMessage());
		}
	}
	 ...
	 * list of parameter classes to use for overload resolution. Method information
	 * is obtained using reflection.
	 *
		...
		if (name.equals("<init>")) {
			...
			for (Constructor<?> con : cs) {
				if (paramTypes == null || Arrays.equals(con.getParameterTypes(), paramTypes)) {
					...
					result = InvokeInstruction.make(computeSignature(con.getParameterTypes(), Void.TYPE), makeType(c),
							name, Dispatch.SPECIAL);
				}
			}
		} else {
			...
			if (m != null) {
				...
				result = InvokeInstruction.make(computeSignature(m.getParameterTypes(), m.getReturnType()), makeType(c),
						name, opcode);
			}
		}
	...
	public static InvokeInstruction makeInvokeSpecial(IMethod m) {
		InvokeInstruction result = null;
		if (m != null) {
			Dispatch opcode = Dispatch.VIRTUAL;
			if (m.isStatic()) {
			...
			result = InvokeInstruction.make(computeSignature(m), makeType(m.getDeclaringClass().getName().toString()),
					m.getReference().getName().toString(), opcode);
		}

		if (result == null) {
			throw new IllegalArgumentException(
					"Method " + m.getName().toString() + " is not present in class " + m.getDeclaringClass().getName().toString());
		} else {
			return result;
		}
	}

	public static InvokeInstruction makeInvokeDefalutInit(IClass c) {
		InvokeInstruction result = null;
		result = InvokeInstruction.make("()V", makeType(c), "<init>", Dispatch.SPECIAL);
		return result;
	}

	public static InvokeInstruction makeInvokeSpecialInit(IMethod m, IClass c) {
		InvokeInstruction result = null;
		String sigString = m.getSignature();
		String argumentString = sigString.substring(sigString.indexOf("(") + 1, sigString.indexOf(")"));
		result = InvokeInstruction.make("(" + argumentString + ")V", makeType(c), "<init>", Dispatch.SPECIAL);
		return result;
	}

	public static InvokeInstruction makeInvokeSetPrimitiveArugumentsMethod(IClass c, String name, String type) {
		InvokeInstruction result = null;
		result = InvokeInstruction.make("(" + type + ")V", makeType(c), "set" + upperFirstLatter(name),
				Dispatch.SPECIAL);
		return result;
	}

	public static InvokeInstruction makeInvokeSetRefArugumentsMethod(IClass c, String name, String type) {
		InvokeInstruction result = null;
		result = InvokeInstruction.make("(" + type + ";)V", makeType(c), "set" + upperFirstLatter(name),
				Dispatch.SPECIAL);
		return result;
	...
	/**
	 * Compute the type index constant (Constants.TYPE_...) from the JVM type.
	 * Returns -1 if the type is not one of the predefined constants.
	 */
	...
	public static Pair<boolean[], boolean[]> computeBasicBlocks(IInstruction[] instructions,

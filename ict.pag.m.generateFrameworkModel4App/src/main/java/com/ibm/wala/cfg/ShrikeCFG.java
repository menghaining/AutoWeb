...
public class ShrikeCFG extends AbstractCFG<IInstruction, ShrikeCFG.BasicBlock> implements BytecodeCFG {
	...
	private void makeBasicBlocks() {
		...
		for (int i = 0; i < instructions.length; i++) {
			...
			if (instructions[i].isPEI() && i < handlers.length) {
	...
	public final class BasicBlock extends NodeWithNumber implements IBasicBlock<IInstruction> {
		...
		protected void addExceptionalEdges(IInstruction last) {
			...
			if (last.isPEI()) {
				...
				if (last instanceof ThrowInstruction) {
					...
					if (hs != null && hs.length > 0) {
						...
						if (last instanceof IInvokeInstruction) {
							...
							MethodReference target = MethodReference.findOrCreate(l, loader.getReference(),
									call.getClassType(), call.getMethodName(), call.getMethodSignature());
							try {
				...
				if (hs != null && hs.length > 0) {
					...
					// this var gets set to false if goToAllHandlers is true but some enclosing
					// exception
					// handler catches all
					// exceptions. in such a case, we need not add an exceptional edge to the method
					// exit
					...
					for (ExceptionHandler element : hs) {
						...
						if (goToAllHandlers) {
							...
							// if the handler catches all exceptions, we don't need to add an edge to the
							// exit or
							// any other handlers
							...
							if (element.getCatchClass() != null) {
								ClassLoaderReference loader = element.getCatchClassLoader() == null
					...
					if ((exceptionTypes == null && needEdgeToExitForAllHandlers)
							|| (exceptionTypes != null && !exceptionTypes.isEmpty() && needEdgeToExitForAllHandlers)) {
						BasicBlock exit = exit();
		...
		private ExceptionHandler[] getExceptionHandlers() {
			...
			int index = getLastInstructionIndex();
...
//			ExceptionHandler[] hs = handlers[getLastInstructionIndex()];
		...
		public String toString() {
			return "BB[Shrike]" + getNumber() + " - " + method.getDeclaringClass().getReference().getName() + '.'
					+ method.getName();
		}

...
function bench() {
    ...
    ID="$BENCHMARK-mhn0202-withoutJackEETestcase-$ANALYSIS"
    local FACTS="$BENCHMARK-facts"
    pushd ${DOOP_HOME}
    set -x
    ${DOOP} --Xstart-after-facts "/home/pldi2020/benchmark-facts/${FACTS}" -a ${ANALYSIS} --id ${ID} --souffle-jobs 16 --open-programs mhn0202-withoutJackEETestcase  --Xsymlink-cached-facts --timeout 100000|& tee ${ID}.log
    set +x
    popd
